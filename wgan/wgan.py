...
class Generator(nn.Module):
    ...
    def __init__(self, num_channels: int, image_size: int, latent_dim: int) -> None:
        ...
        self.convs = nn.Sequential(
            nn.BatchNorm2d(1024),
            nn.ConvTranspose2d(
                1024, 512, kernel_size=5, stride=2, padding=2, output_padding=1
            ),
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.ConvTranspose2d(
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(
                128, num_channels, kernel_size=5, stride=2, padding=2, output_padding=1
            ),
            nn.Tanh(),
        )
...
class Discriminator(nn.Module):
    ...
    def __init__(self, num_channels: int, image_size: int) -> None:
        ...
        self.model = nn.Sequential(
            nn.Conv2d(num_channels, 128, kernel_size=5, stride=2, padding=2),
            nn.LeakyReLU(0.2),
            ...
            nn.LeakyReLU(0.2),
            nn.Flatten(),
            nn.Linear(1024 * (image_size // 16) ** 2, 1),
        )
